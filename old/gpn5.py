...
class GrowingPlanarNetwork(CheckerPlanarNetwork):
    ...
    def _make_edge(self, dual_node, min_pair):
        ...
        assert len(set(ingbs)) == len(ingbs), f"Error found pair in {ingbs} with " \
            f"{map_edge_pair} and {ordered_cycle_pairs}"
        
        assert split_ngb_2 in ingbs, f"Error couldn't find {split_ngb_2} in {ingbs} with " \
            f"{map_edge_pair} and {ordered_cycle_pairs}"
        ...
        assert split_ngb_1 in ingbs, f"Error couldn't find {split_ngb_2} in {ingbs} with " \
            f"{map_edge_pair} and {ordered_cycle_pairs}"
    ...
    def common_dual(self, edge_1, edge_2):
        ...
        if len(x) > 1:
            raise ValueError(f"Found multiple common dual"
                             f"{x} for edges {edge_1} and {edge_2}")
    ...
    def outside_with_anchor(self, node, ngb1, ngb2):
        ...
        if len(self.ngb(current)) <= 2:
            raise RuntimeError(f"{current} has not enough neighbours, requires at least 3")
