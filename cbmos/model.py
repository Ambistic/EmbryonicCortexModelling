...
class Brain:
    ...
    def __init__(
        ...
        # init
        self.tissue = CBModelv2(ff.Gls(), ef.solve_ivp, dimension=2,
    ...
    def _tick(self, absolute_time, relative_time):
        ...
        self.snapshots.append(self.tissue.export())
        return True

    def _sanity_check(self):
        if len(self.population) > 1e4:
    ...
    def divide(self, cell, T):
        ...
        """
        self.debug("Duplicating " + str(cell.index) + " " + str(cell.tissue_id))
        new_tissue_id = self.tissue.divide_cell(cell.tissue_id)
        time_ = cell.appear_time + cell.eff_Tc
        # time_ = T
        new_cell_1 = cell.generate_daughter_cell(time_, index=self.new_cell_id(),
                tissue_id=new_tissue_id)
