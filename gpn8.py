...
class GrowingPlanarNetwork(CheckerPlanarNetwork):
        ...
        for i in range(size):
            for j in range(size):
                ...
                if j > 0:
                    ...
                    idx = self.add_edge(D, *D_edge, dual=G_edge)
                    self.add_edge(G, *G_edge, dual=(*D_edge, idx))
    ...
    def _duplicate_node(self, node):
        ...
        # A2 remove edge
        for ngb in ngbs:
            self.remove_edge(self.G, node, ngb)
        ...
        for i in range(half_1):
            self.add_edge(self.G, node, current, dual=cycle_dual[current])
        ...
        for j in range(half_2):
            self.add_edge(self.G, new_node, current, dual=cycle_dual[current])
            try:
        ...
        idx = self.add_edge(self.D, first_dual, second_dual, dual=(node, new_node))
        inner_dual_edge = Ltuple((first_dual, second_dual, idx))
        self.add_edge(self.G, node, new_node, dual=inner_dual_edge)
    ...
    def _swap_node(self, node1, node2):
        ...
        def ngb_update(self, nodeA, nodeB, ngbs):
            for ngb in ngbs:
                ...
                self.add_edge(self.G, nodeB, ngb, dual=dual_edge)
                self.remove_edge(self.G, nodeA, ngb)
    ...
    def _make_edge(self, dual_node, min_pair):
        ...
        while current != split_ngb_2:
            ...
            self.add_edge(self.D, *dual_edge, dual=planar_edge)
        ...
        while current != split_ngb_1:
            ...
            idx = self.add_edge(self.D, *current_edge, dual=planar_edge)
        ...
        idx = self.add_edge(self.D, dual_node, new_node)
        ...
        self.add_edge(self.G, split_ngb_1, split_ngb_2, dual=inner_dual_edge)
    ...
    def _remove_node(self, node):
        ...
        for new_ingb, old_edge in zip(new_ingbs, old_edges):
            idx = self.add_edge(self.D, keeped_dual, new_ingb)
            ...
            # remove old edge
            self.remove_edge(self.D, *old_edge)
    ...
    def _remove_edge(self, edge):
        ...
        for i_dual_edge in ls_1:
            ...
            self.remove_edge(self.D, *i_dual_edge)
            idx = self.add_edge(self.D, dual_0, i_dual_node)
    ...
    def _quick_remove(self, node):
        ...
        # 1
        ngbs = self.ngb(node)
        self.add_edge(self.G, ngbs[0], ngbs[1])

        # 2
        keeped_dual_edge = self.dual((node, ngbs[0]))
        removed_dual_edge = self.dual((node, ngbs[1]))
        self.remove_edge(self.D, *removed_dual_edge)

        # 3
        self.remove_edge(self.G, node, ngbs[0])
        self.remove_edge(self.G, node, ngbs[1])
    ...
    def _make_border_edge(self, node, min_pair):
        ...
        self.remove_edge(self.D, *self.dual(crossing_pairs[0]))
        self.remove_edge(self.D, *self.dual(crossing_pairs[1]))
    ...
    def stabilize_ngb(self, node):
        ...
        # MIN NGB
        # don't use side, which is actualized later
        # rather use 1 if is border
        is_border = 1 if self.is_border_node(node) else 0
        # if len(ngbs) + len(sides) < 3:
        if len(ngbs) + is_border < 3:
