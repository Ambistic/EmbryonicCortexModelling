...
class RelaxableGPN(GrowingPlanarNetwork):
    ...
    def quick_update_dist(self, direction="N"):
        ...
        for n in nodes:
            # value of dist, node n, whether it's a first trial
            heappush(queue, (value, n, True))
        ...
        while queue and nodes:
            ...
            if is_first:
                val = self.collect_dist(inode, key)
                if val != -1:
                    heappush(queue, (value, inode, False))
                    continue
    ...
    def collect_dist(self, inode, key):
        ...
        for ngb in node["ngb"]:
            if value > self.G.nodes[ngb].get(key, np.inf) + 1:
                self.G.nodes[inode][key] = value
                has_changed = True
        return has_changed
