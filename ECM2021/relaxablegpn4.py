...
class RelaxableGPN(GrowingPlanarNetwork):
    ...
    def destroy(self, node, relax=False):
        ...
        for ngb in ngbs:
            ...
            # note that this shall be prevented by tracking non border <3ngbs
            # because this might cause problem with other borders (maybe not)
        self.callback()
    ...
    def relax(self):
        ...
        for n in nodes:
            ...
            if self.low_width_penalty(dist_NS) != 0:
            ...
            if self.low_width_penalty(dist_EW) != 0:
        ...
        for pt in candidates[:4]:
            ...
            for action in self.enumerate_actions(pt):
                tot_action += 1
                if tot_action > 100:
                    raise RuntimeError("Too many actions, generally this means that a cycle is too large")
    ...
    def estimate_action(self, action, mean_NS, mean_EW):
        # information about whether some nodes become border or not border
        # is not important because keeping same information allow finding better
        # situation (reducing does reduce on the border, while augmenting will
        # show a +1 for already borders and new border information is not necessary)
        old_score_NS = []
        new_score_NS = []
        old_score_EW = []
        new_score_EW = []
    ...
    def keep_best_trial(self):
        ...
        trials = [self] + self.trials
        scores = [x.get_main_metric(update=True) for x in trials]
