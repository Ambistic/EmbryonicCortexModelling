...
def preprocess_progeny(param=None, root=None, history=None):
    if history is None:
        history = preprocess_history(param, root)
    return progeny_along_time(history)

def preprocess_progeny_all(param=None, root=None, history=None):
    if history is None:
        history = preprocess_history(param, root)
...
def mean_sd(ls):
    ...
    x = list(ls[0]["x"])[:min_len]
    mean = np.mean([list(sample["y"])[:min_len] for sample in ls], axis=0)
    sd = np.std([list(sample["y"])[:min_len] for sample in ls], axis=0)
...
def preprocess_corr_metrics(param=None, root=None, history=None):
    if history is None:
        history = preprocess_history(param, root)
