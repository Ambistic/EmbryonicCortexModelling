...
class Cell2:
    def __init__(self, grn: GRNMain2, quantities=None):
        self.grn = grn
    ...
    def run_step(self, ts=0.1):
        self.quantities, self.expression, self.derivative = self.grn.run_step(
    ...
    def divide(self):
        ...
        return Cell2(self.grn, quantities=jnp.array(q1)), Cell2(self.grn, quantities=jnp.array(q2))
...
@jit
def batch_compute_expression(ls_quantities, b, theta, m):
    return [j_gene_expression(q, b, theta, m) for q in ls_quantities]


def batch_compute_derivatives(genes, expressions):
    gene_funcs = [gene.tree.get_compiled() for gene in genes]
    return j_batch_compute_derivatives(gene_funcs, expressions)


# @jit
def j_batch_compute_derivatives(gene_funcs, expressions):
    return [
...
class CellBatch:
    ...
    def run_step_old(self, ts=0.1):
        ...
        expressions = batch_compute_expression(quantities, self.grn.b, self.grn.theta, self.grn.m)
        derivatives = batch_compute_derivatives(self.grn.genes, expressions)
