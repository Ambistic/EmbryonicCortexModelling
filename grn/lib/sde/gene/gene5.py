...
from lib.sde.util.initializer import generate_random_init, generate_random_noise, generate_random_expr2, \
    ...
    generate_random_tree_2, generate_random_asym


class MetaParamGene5(type):
...
class ParamGene5(metaclass=MetaParamGene5):
    init = 0
    noise = 1
    b = 2
    m = 3
    ...
    @staticmethod
    def get(name):
        return getattr(ParamGene5, name)
...
class GeneMain5:
    def __init__(self, idx, nb_genes, nb_membrane_gene, params, init=True, tree=None,
        ...
        if tree is not None:
            ...
            self.tree = generate_random_tree_2(self.get_all_labels(), max_depth=2, prob_gene=0.5)
    ...
    def __setattr__(self, key, value):
        if key in ParamGene5:
            self.params[ParamGene5.get(key), self.idx] = value
        else:
            object.__setattr__(self, key, value)

    def __getattr__(self, item):
        if item in ParamGene5:
            return self.params[ParamGene5.get(item), self.idx]
        else:
    ...
    def get_info(self, cell=None):
        dict_info = {
            var: getattr(self, var) for var in ParamGene5
        }
